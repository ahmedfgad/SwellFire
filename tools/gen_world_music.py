...
def build_boss():
    """Boss — epic battle, tense A minor, brass + stabs + hard 16th drums."""
    bpm = 140.0
    n = _bar_n(bpm)
    beats = n / SAMPLE_RATE / (60.0 / bpm)
    # A minor: Am F G Em power/triad pads
    chord_beats = [(0, 4, [45, 57, 60]), (4, 4, [41, 53, 57]),
                   (8, 4, [43, 55, 59]), (12, 4, [40, 52, 55])]
    chord_beats = [(b, d, m) for (b, d, m) in chord_beats if b < beats]
    pad = _layered_pad(n, bpm, chord_beats)
    # tense ascending brass motif
    mel = [(0, 1, 57), (1, 1, 60), (2, 1, 62), (3, 1, 64), (4, 2, 65),
           (6, 2, 62), (8, 1, 60), (9, 1, 62), (10, 1, 64), (11, 1, 65),
           (12, 1, 67), (13, 1, 68), (14, 2, 69)]
    mel = [(b, d, m) for (b, d, m) in mel if b < beats]
    brass = seq(n, bpm, mel, _brass_voice)
    # rhythmic square stabs on the off-beats
    stabs = [(b + 0.5, 0.25, [45, 52]) for b in range(int(beats))]
    stab = seq(n, bpm, stabs, _pluck_voice)
    # hard driving 16th drums
    pat = []
    for b in range(int(beats)):
        pat.append((b, "k"))
        if b % 2 == 1:
        ...
        for q in (0.25, 0.5, 0.75):
            pat.append((b + q, "h"))
    drums = _drum_track(n, bpm, pat)
    return normalize(mix(pad * 0.4, brass * 0.6, stab * 0.4, drums * 0.55), 0.9)
