import os, sys
sys.path.insert(0, os.path.join(os.path.dirname(__file__), "..", ".."))
os.environ.setdefault("SDL_AUDIODRIVER", "dummy")


def test_fps_is_60():
    from tools import make_videos
    assert make_videos.FPS == 60


def test_segments_one_regular_level_for_worlds_1_to_4():
    from tools import make_videos
    segs = make_videos._segments()
    levels = [s[1] for s in segs]
    assert levels == [6, 16, 26, 36]
    # all non-boss (boss is level 10 of each world)
    assert all(lvl % 10 != 0 for lvl in levels)


def test_segment_frame_cap_covers_real_time_length():
    """The safety cap must exceed the level's real-time frame count (so a full
    real-time playthrough is never truncated mid-level)."""
    import levels
    from tools import make_videos
    for label, level, warmup, frames in make_videos._segments():
        secs = (levels.get_level(level)["distance_goal"]
                / levels.SCROLL_SPEED_PX_PER_SEC)
        real_frames = secs * make_videos.FPS
        assert frames >= real_frames + warmup, (level, frames, real_frames)
