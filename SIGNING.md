...
GateRunner is signed for Google Play with an upload key. This document describes the keystore the build script creates, how Play App Signing fits in, and what to do if you ever lose the upload key.
...
- `gaterunner-upload.keystore` — the PKCS12 keystore with the upload key.
...
When you create the GateRunner app in Play Console for the first time, **enable Play App Signing** during the initial release flow. Google then holds the real signing key and you only need to protect the upload key. If the upload key is ever lost, Google can reset it — the app on Play stays signed by the same (Google-held) key, so existing users keep getting updates as normal.
...
1. Run `./build_android.sh` once. It creates a new `gaterunner-upload.keystore` and `upload_certificate.pem`.
...
1. Change the id in `buildozer.spec`, for example:
   ...
   ```
   package.name = gaterunner2
   package.domain = com.ahmedgad
   ```

   This gives the id `com.ahmedgad.gaterunner2`.
...
- `gaterunner-upload.keystore` — the upload key.
