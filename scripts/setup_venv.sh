...
#
# Options:
#   ./scripts/setup_venv.sh                     full setup
#   ./scripts/setup_venv.sh --dev               install all development and test dependencies
#   ./scripts/setup_venv.sh --no-android        skip the Android build tools
#   ./scripts/setup_venv.sh --skip-system-deps  only create the venv, no apt, no sudo
#
set -euo pipefail
...
echo "Run the game without audio (some VMs):  SDL_AUDIODRIVER=dummy python main.py"
echo "Run without screen:  xvfb-run -a python main.py"
[[ "$INSTALL_ANDROID" -eq 1 ]] && echo "Build for Android:   ./scripts/build_android.sh"
