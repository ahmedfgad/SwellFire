...
#   3. Checks that the Android build tools are present (installed by scripts/setup_venv.sh).
...
#
# Options:
#   ./scripts/build_android.sh             build the release .aab and .apk
#   ./scripts/build_android.sh --debug     build a quick unsigned debug .apk only
#   ./scripts/build_android.sh --skip-deps do not check the system build tools
#   ./scripts/build_android.sh --check     validate config only; do not build or sign
#
...
if [[ ! -d "$VENV_DIR" ]]; then
    echo "venv missing, creating it with scripts/setup_venv.sh"
...
# Check the Android build tools. scripts/setup_venv.sh installs them.
if [[ "$SKIP_DEPS" -eq 0 ]]; then
    ...
    if [[ -n "$missing" ]]; then
        ...
        echo "Run ./scripts/setup_venv.sh first, then run this script again." >&2
        exit 1
    fi
fi
...
echo "  For signing or lost key questions, see docs/platforms/android-signing.md."
